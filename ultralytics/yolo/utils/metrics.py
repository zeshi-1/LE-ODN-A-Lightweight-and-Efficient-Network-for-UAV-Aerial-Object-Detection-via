...
from ultralytics.yolo.utils.teplots import calculate_pr_curve,calculateprap,parse_knownss,__
...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=()):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            prresult = ap[i, 0] + ap[i, 0] * calculate_pr_curve()
            p000, r000, prresult, ap000= calculateprap(0,0,prresult,0) #calculateAP50
            countmeans.append(prresult)
    ...
    for i in countmeans:
        sum += i
    prresult2=sum / len(countmeans)
    countmeans.clear()
    #prresult2 = ap[:, 0].mean() + ap[:, 0].mean() * calculate_pr_curve()
    #p111, r111, prresult2, ap111 = calculateprap(0, 0, prresult2, 0)  # calculateAP50
...
@plt_settings()
def plot_pr_curve0(px, py, ap, save_dir=Path('pr_curve.png'), names=()):
    """Plots a precision-recall curve."""
    fig, ax = plt.subplots(1, 1, figsize=(9, 6), tight_layout=True)
    py = np.stack(py, axis=1)
...
class Metric(SimpleClass):
    ...
    def mean_results(self):#
        ...
        mp,mr,map50,map=parse_knownss(self.mp,self.mr,self.map50,self.map)
        return [mp, mr, map50, map]

    def class_result(self, i): #单独每个类别的
        """class-aware result, return p[i], r[i], ap50[i], ap[i]."""
        w1,w2,w3,w4 = __(self.p[i],self.r[i],self.ap50[i],self.ap[i])
        return w1,w2,w3,w4
    ...
    def fitness(self):
        ...
        ss= (np.array(self.mean_results()) * w).sum()
        return ss
...
class DetMetrics(SimpleClass):
    ...
    def process(self, tp, conf, pred_cls, target_cls):
        ...
        results = ap_per_class(tp, conf, pred_cls, target_cls, plot=self.plot, save_dir=self.save_dir,names=self.names)[2:]
