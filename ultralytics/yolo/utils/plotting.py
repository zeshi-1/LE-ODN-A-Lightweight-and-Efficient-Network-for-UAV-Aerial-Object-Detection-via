...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA) #thickness=self.lw#########################################
            if label:
                tf = max(self.lw - 1, 2)  # font thickness
                w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 4, thickness=tf)[0]  # text width, height################################################fontScale=self.lw / 0.5
                outside = p1[1] - h >= 3
                ...
                cv2.putText(self.im,
                            ...
                            0,
                            self.lw / 4,##########################################################################################################################self.lw / 0.5
                            txt_color,
                            thickness=tf,
                            lineType=cv2.LINE_AA)
        # else:  # cv2
        #     p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
        #     cv2.rectangle(self.im, p1, p2, color, thickness=20, lineType=cv2.LINE_AA) #thickness=self.lw#########################################
        #     if label:
        #         tf = max(self.lw - 1, 20)  # font thickness
        #         w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 0.5, thickness=tf)[0]  # text width, height################################################fontScale=self.lw / 0.5
        #         outside = p1[1] - h >= 3
        #         p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
        #         cv2.rectangle(self.im, p1, p2, color, -1, cv2.LINE_AA)  # filled
        #         cv2.putText(self.im,
